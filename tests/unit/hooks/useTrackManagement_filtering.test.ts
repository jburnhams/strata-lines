...
jest.mock('@/utils/colorAssignment', () => ({
    assignTrackColors: jest.fn((tracks: any[]) => tracks.map(t => ({ ...t, color: '#ff0000' }))),
}));
...
describe('useTrackManagement Filtering', () => {
    ...
    beforeEach(() => {
        ...
        (db.getTracks as any).mockResolvedValue(mockTracks);

        global.URL.createObjectURL = jest.fn((obj: Blob | MediaSource) => 'blob:mock-url') as unknown as typeof URL.createObjectURL;
    ...
    it('should download only filtered tracks', async () => {
        ...
        (mockGenerateAsync as any).mockResolvedValue(new Blob([]));
